export default (config = {}) => ({
    ...
    async fetchData() {
        try {
            ...
            const spotPriceResponse = await axios.get("https://mwc2.pacificpool.ws/api/price-indexes/spot_price_mwc");
            // this.spotPriceData = this.formatDataForChart(spotPriceResponse.data, "Spot Price");

            // Fetch current difficulty data
            const response = await axios.get("https://mwc2.pacificpool.ws/api/price-indexes-background/block-data");
    ...
    drawSpotPriceChart() {
        const options = {
            series: [{
                ...
                data: this.currentDifficultyData
            }],
            chart: {
                type: 'line',
                height: 350
        ...
        const chart = new ApexCharts(this.$refs.spotPriceChart, options);
        chart.render();
    },
